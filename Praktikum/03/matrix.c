...
*/
{
   int ll = ROW_EFF(*m)/2-1;

   int i,j,k;
   for (i = 0; i <= ll; i++)
   {
      ElType temp = ELMT(*m,i,i);
      for (k = 1+i; k < COL_EFF(*m)-i; k++)
      {
         ELMT(*m,i,k) ^= temp;
         temp ^= ELMT(*m,i,k);
         ELMT(*m,i,k) ^= temp;
      ...
      for (j = 1+i; j < ROW_EFF(*m)-i; j++)
      {
         ELMT(*m,j,getLastIdxCol(*m)-i) ^= temp;
         temp ^= ELMT(*m,j,getLastIdxCol(*m)-i);
         ELMT(*m,j,getLastIdxCol(*m)-i) ^= temp;
      }

      for (k = getLastIdxCol(*m)-1-i; k >= i; k--)
      {
         ELMT(*m,getLastIdxRow(*m)-i,k) ^= temp;
         temp ^= ELMT(*m,getLastIdxRow(*m)-i,k);
         ELMT(*m,getLastIdxRow(*m)-i,k) ^= temp;
      }

      for (j = getLastIdxRow(*m)-1-i; j >= i; j--)
      {
         ELMT(*m,j,i) ^= temp;
         temp ^= ELMT(*m,j,i);
         ELMT(*m,j,i) ^= temp;
