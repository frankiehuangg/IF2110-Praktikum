...
{
   POINT p1; BacaPOINT(&p1);
   POINT p2; BacaPOINT(&p2);
...
/* rumus: sqrt((Absis(P1)-Absis(P2))^2 + Ordinat(P1)-Ordinat(P2)^2) */
{
...
/* Prekondisi : x1 != x2 */
{
   float num = Ordinat(PAkhir(L))-Ordinat(PAwal(L));
   float den = Absis(PAkhir(L))-Absis(PAwal(L));
   return num/den;
}
